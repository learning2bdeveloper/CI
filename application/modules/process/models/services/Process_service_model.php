...
class Process_service_model extends CI_Model
{
    ...
    public function save_method_from_model()
    {
        ...
        try {
            ...
            $this->db->insert($this->Table->process, $data); // Table-> nd Table[''] kay bawal array 
    ...
    private function checkProcessDocuments($processID)
    {
        $query = $this->db->from($this->Table->clientprocess)
    ...
    public function delete_method_from_model()
    {
        ...
        if ($this->checkProcessDocuments($this->processID)) {
            return array("message" => "Can't delete there are documents registered!", "has_error" => true);
        }

        try {
            $this->db->trans_start();
            $this->db->where('ProcessID', $this->processID);
            $this->db->delete($this->Table->process);
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                $this->db->trans_rollback();
                throw new Exception(ERROR_PROCESSING, true);
                ...
                //$this->db->trans_commit();
                return array('message' => "Process deleted!", 'has_error' => false);
            }
        } catch (Exception $msg) {
            return array('message' => $msg->getMessage(), 'has_error' => true);
        }
