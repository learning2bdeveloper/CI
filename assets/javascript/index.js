if (document.readyState !== 'loading') {
    ...
    const reloadClientTable = async () => {
        //C:\xampp\htdocs\kyanu_document_tracking\application\modules\create_organization\views\grid\load_organization  create_organization/grid/load_organization
        const response = await fetch('Create_client/get_client_info');
        const data = await response.text();
    ...
    async function deleteClientData(value) {
        ...
        if(confirmation) {
            try 
                ...
                let data = new FormData();
                data.append('id', value);
                const response = await fetch('create_client/services/Create_client_service/deleteinfo', {
                    method: 'POST',
                    body: data
                });
